...
function b(){ const fU=new n("New_1");const fV=(fU.h.push(q(fU.k, "registerhoudini", () => fU.i, () => { const f1=null;P(M(() => N(s(d()), () => O(null))), null);})),fU);const fW=(fV.h.push(new H("status", c().View)),fV);const fX=r(fW.k, fW.h, []);const fY=new i(fX[1], F(fX[0]));let fZ=(fW.i=fY,fY);const f0=fZ.Doc;u0031("");bi.RunById("main", f0);}
function c(){ return I.statusMessage;}
function d(){ const fU=null;return new Promise((fV) => { fV(new Promise((fW) => { fW(globalThis.CSS.paintWorklet.addModule("paint-worklet.js").then(() => { c().Set("\u2705 Houdini Paint Worklet Registered!");return Promise.resolve(null);}));})["catch"]((fW) => { const fX=t(fW);c().Set("\u274c Error: "+String(fX.message));return Promise.resolve(null);}));});}
...
let I=fO((fU) => class fV { static { I=fU(this);} static statusMessage;static { this.statusMessage=bk.Create_1("Click the button to register a custom Houdini paint worklet.");} });
